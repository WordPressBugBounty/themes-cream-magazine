...
class Cream_Magazine {
	...
	public function setup() {
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'cream_magazine_custom_background_args',
				array(
		...
		add_theme_support(
			'custom-header',
			apply_filters(
				'cream_magazine_custom_header_args',
				array(
					'default-image'      => '',
	...
	public function header_style() {
		...
		<style type="text/css">
			<?php
			if ( ! display_header_text() ) { // If header text is hidden.
				?>
				.site-title,
				.site-description {
					position: absolute;
	...
	public function enqueue_scripts() {
		...
		if ( is_rtl() ) {
			...
			wp_enqueue_style(
				'cream-magazine-main-rtl',
				get_template_directory_uri() . '/assets/dist/css/main-rtl.css',
				null,
				CREAM_MAGAZINE_VERSION,
				'all'
			);
		...
		$cream_magazine_script_obj = array(
			'show_search_icon'           => cream_magazine_get_option( 'cream_magazine_enable_search_button' ),
